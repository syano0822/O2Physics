...
#include "MFTMuonMatching/MftMuonMatchingUtils.h"
...
struct match_mft_mch_data_mc {
  ...
  Configurable<bool> fUseMuonInfoMFTExtrap{"cfgUseMuonInfoMFTExtrap", 1, ""};
  ...
  Configurable<float> fPreselectMatchingEta{"cfgPreselectMatchingEta", 1.f, ""};
  Configurable<float> fPreselectMatchingPhi{"cfgPreselectMatchingPhi", 1.f, ""};
  ...
  Configurable<float> fDeviationTagMuonCut{"cfgDeviationTagMuonCut", 2.f, ""};
  ...
  {
    float dxNorm = (mDX - fMeanXTagMuonCut) / (fSigmaXTagMuonCut * fDeviationTagMuonCut);
    float dyNorm = (mDY - fMeanYTagMuonCut) / (fSigmaYTagMuonCut * fDeviationTagMuonCut);
    float detaNorm = (mDEta - fMeanEtaTagMuonCut) / (fSigmaEtaTagMuonCut * fDeviationTagMuonCut);
    float dphiNorm = (mDPhi - fMeanPhiTagMuonCut) / (fSigmaPhiTagMuonCut * fDeviationTagMuonCut);
  ...
  inline bool isPassMatchingPreselection(float Dx, float Dy, float Deta, float Dphi)
  {
    return !(std::abs(Dx) > fPreselectMatchingX || std::abs(Dy) > fPreselectMatchingY);
    return !(std::abs(Deta) > fPreselectMatchingX);
    return !(std::abs(Dphi) > fPreselectMatchingPhi);
  }
  ...
  Produces<o2::aod::MixProbeMatchParams> tableMixProbeMatchingParams;

  Produces<o2::aod::TagMatchParams> tableTagMatchingParams;

  Produces<o2::aod::MuonPair> tableMuonPair;
  Produces<o2::aod::TagProbeMuonPair> tableTagProbeMuonPair;
    ...
    for (auto const& map_collision : map_collisions) {
      ...
      for (auto const& imuontrack1 : map_muontracks[map_collision.first]) {
        ...
        for (auto const& imfttrack1 : map_mfttracks[map_collision.first]) {
          ...
          o2::mft::MftMuonMatchingUtils<MyMUON, MyMFT, MyCollision> matching(muontrack1, mfttrack1, collision, fMatchingMethod, fUseMuonInfoMFTExtrap, fieldB);
          matching.calcMatchingParams();

          if (!isPassMatchingPreselection(matching.getDx(), matching.getDy(), matching.getDeta(), matching.getDphi()))
            continue;
          ...
          bool isTrue = false;
          //
          tableMatchingParams(map_collision.first,
                              matching.getGMPtAtDCA(),
        ...
        for (auto const& map_mfttrack : map_mfttracks) {
          ...
          for (auto const& imfttrack1 : map_mfttrack.second) {
            ...
            tableMixMatchingParams(map_collision.first,
                                   matching.getGMPtAtDCA(),
                                   matching.getGMEtaAtDCA(),
                                   static_cast<int16_t>(matching.getGMQ()),
                                   matching.getDpt(),
        ...
        for (auto const& imuontrack2 : map_muontracks[map_collision.first]) {
          ...
          o2::mft::MatchingTagAndProbe<MyMUON, MyCollision> tagdimuon(muontrack1, muontrack2, collision);
          ...
          if (isGoodTagDimuon(tagdimuon.getMass())) {
            for (auto const& imfttrack1 : map_mfttracks[map_collision.first]) {
              auto const& mfttrack1 = mfttracks.rawIteratorAt(imfttrack1);
              o2::mft::MftMuonMatchingUtils<MyMUON, MyMFT, MyCollision> matchingTag(tagmuontrack, mfttrack1, collision, fMatchingMethod, fUseMuonInfoMFTExtrap, fieldB);
              ...
              if (isGoodTagMatching(matchingTag.getDx(), matchingTag.getDy(), matchingTag.getDeta(), matchingTag.getDphi()) &&
                  isPassMatchingPreselection(matchingTag.getDx(), matchingTag.getDy(), matchingTag.getDeta(), matchingTag.getDphi())) {
                bool isTrue = false;
                tableTagMatchingParams(map_collision.first,
                                       matchingTag.getGMPtAtDCA(),
          ...
          tableTagProbeMuonPair(tagdimuon.getCharge(), tagdimuon.getMass(), tagdimuon.getPt(), tagdimuon.getRap());

          for (auto const& imfttrack1 : map_mfttracks[map_collision.first]) {
            auto const& mfttrack1 = mfttracks.rawIteratorAt(imfttrack1);
            if (mfttrack1.globalIndex() == IndexTagMFTCand)
              continue;
            o2::mft::MftMuonMatchingUtils<MyMUON, MyMFT, MyCollision> matchingProbe(probemuontrack, mfttrack1, collision, fMatchingMethod, fUseMuonInfoMFTExtrap, fieldB);
            matchingProbe.calcMatchingParams();

            if (isPassMatchingPreselection(matchingProbe.getDx(), matchingProbe.getDy(), matchingProbe.getDeta(), matchingProbe.getDphi())) {
          ...
          if (minimumIndexProbeMFTCand > -1) {
            ...
            tableProbeMatchingParams(map_collision.first,
                                     probeMatchingParams[0],
            ...
            // cout<<probeMatchingParams[5]<<"   "<<probeMatchingParams[6]<<"   "<<probeMatchingParams[7]<<"   "<<probeMatchingParams[8]<<endl;
            ...
            for (auto const& map_mfttrack : map_mfttracks) {
              ...
              for (auto const& imfttrack1 : map_mfttrack.second) {
                auto const& mfttrack1 = mfttracks.rawIteratorAt(imfttrack1);
                o2::mft::MftMuonMatchingUtils<MyMUON, MyMFT, MyCollision> matching(probemuontrack, mfttrack1, collision, fMatchingMethod, fUseMuonInfoMFTExtrap, fieldB);
                matching.calcMatchingParams();
                ...
                tableMixProbeMatchingParams(map_collision.first,
                                            tagGMPtAtDCA,
                                            matching.getGMPtAtDCA(),
                                            matching.getGMEtaAtDCA(),
                                            matching.getGMQ(),
                                            ...
                                            matching.getDphi(),
                                            isTrue);
              } // endo of loop imfttrack1
            } // end of loop map_mfttrack

          } // end of if (minimumIndexProbeMFTCand > -1)
